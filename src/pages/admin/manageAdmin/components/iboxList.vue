...
export default {
  ...
  data() {
    return {
      ...
      areaId: "",
      platform: "",
      palatformId: localStorage.getItem("systembIdHse"),
      platformlist: [],
      platformlistold:[],
      platformold:"",
      platformoldId:localStorage.getItem("systembIdHse")//编辑里面的平台选择id
    };
  },
  methods: {
    ...
    resect() {
      ...
      this.platformold = "";
      this.platformoldId = localStorage.getItem("systembIdHse");
      // this.schoolList1 = [];
    ...
    getSchools() {
      Vue.http.headers.common["userToken"] = getCookie("userToken");
      let _url = this.global.getSchools+'?systembId='+ localStorage.getItem("systembIdHse");
    ...
    getSchoolsOne() {
      Vue.http.headers.common["userToken"] = getCookie("userToken");
      let _url = this.global.getSchoolsDrowDown+'?systembId='+ localStorage.getItem("systembIdHse");
      this.$http.get(_url).then(res => {
        if (res.data.status === 200) {
        ...
        this.$http
          .post(
            equipApi,
            {
              ...
              systembId:localStorage.getItem("systembIdHse")
            },
            { emulateJSON: true }
          )
          .then(
