...
export default {
  ...
  methods: {
    ...
    getBoxtaskInfoIdListFn(){
      getBoxtaskInfoIdList(localStorage.getItem("systembIdHse")).then(res => {
         console.log(res)
      })
    },
    getSysCourseListFn(){
      getSysCourseList(localStorage.getItem("systembIdHse")).then(res => {
    ...
    outer(){
      generateSheetForScoreListCJ(localStorage.getItem("systembIdHse")).then(res => {
          if(res.data.status == 200){
